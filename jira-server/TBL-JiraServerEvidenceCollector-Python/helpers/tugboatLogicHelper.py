...
def upload_evidence(evidence_url: str, username: str, 
    ...
    filename : str
    The filepath which contains the Excel file to be uploaded to Tugboat Logic"""
    # here we are going to use basic authentication to authorize 
    # with the Tugbloat Logic Evidence Collector API
    ...
    with open(file_path, 'rb') as file:
        ...
        fields={'collected': datetime.datetime.now().strftime("%Y-%m-%d"),
                # send evidence file (Excel document) here
                'file': ('evidence.xlsx', file.read(), 
                        ('application/vnd.openxmlformats-officedocument'
                        '.spreadsheetml.sheet'))}
        )
        # set the http authorization header to the Base64 value we generated 
        # above assemble the Tugbloat Logic Evidence Collector authorization 
        # header, which is a combination of the username and password, and the 
        # Tugboat API key
        hdr = {'Authorization':f'Basic {b64auth}',
            # definte the api key
            'X-API-KEY':api_key,
            # use this to 'spoof' Tugbloat Logic Evidence Collector which only 
            # allows requests from common browsers
