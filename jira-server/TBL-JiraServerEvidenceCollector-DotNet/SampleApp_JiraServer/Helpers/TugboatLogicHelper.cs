...
namespace SampleApp_JiraServer.Helpers
{
    public class TugboatLogicHelper
    {
        ...
        /// <param name="inputStream">The input stream which contains the Excel file to be uploaded to Tugboat Logic</param>
        ...
        public static async Task<TugboatLogicResponseModel> UploadEvidence(string evidenceUrl, string username, string password, string apiKey, Stream inputStream)
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                {
                    // send evidence collected date value here, which should be the date the Excel document was generated
                    {new StringContent(DateTime.Now.ToString("yyyy-MM-dd")), "collected"},

                    // send evidence file (Excel document) here
                    {new StreamContent(inputStream),"file","evidence.xlsx"}
                };
